...
The original independent runs used clean copies of `baseline/` and upstream Skill head `6e4fdb9a24a4f0880d67c4c983633b19d10e2ebe`. The Change commands and receipts were subsequently rechecked under the explicit closed-world premise. That mechanical recheck is separate from rerunning the Skill's decisions. The historical prompts below retain the upstream `$simplify-codebase` name; use `$code-janitor` when evaluating this package.
